...
from datasets.PersonCar.label_name import English_name  # 从datasets库中导入English_name字典，用于获取类别的英文名称
...
ini_params = {
    'device': device,  # 设备类型，这里设置为默认CPU，有GPU则使用GPU
    'conf': 0.25,  # 物体置信度阈值
...
class YOLOv8v5Tracker(Detector):  # 定义YOLOv8Detector类，继承自Detector类
    def __init__(self, params=None):  # 定义构造函数
        ...
        self.names = list(English_name.values())  # 获取所有类别的中文名称
    ...
    def load_model(self, model_path):  # 定义加载模型的方法
        ...
        self.names = [English_name[v] if v in English_name else v for v in names_dict.values()]  # 将类别名称转换为英文
