...
# Define a list to hold detection data
detections = []
def frame_process(image):
    global track_history,detections  # 使用全局轨迹历史字典
            ...
            if track_id is not None:
                ...
                if len(track_history[track_id]) > 1:
                    ...
                    if class_name == 'people':
                        last_track_info = (class_name,track_id, velocity_x, velocity_y)  # Update last tracked info
            detections.append({
                'Track ID': track_id,
                'Video Directory': filename,  # Or the actual video path variable you want to use
...
if __name__ == '__main__':  # 如果当前模块是主模块
    ...
    # After exiting the app, write the detections to a CSV file
    df_detections = pd.DataFrame(detections)
    now_time = time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime(time.time()))
    csv_path = abs_path(f"detections_{now_time}.csv", path_type="current")
    df_detections.to_csv(csv_path, index=False)
    print(f"Detections have been written to {csv_path}")
